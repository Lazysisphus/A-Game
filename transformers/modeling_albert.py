...
@add_start_docstrings(
    """Albert Model transformer with a sequence classification/regression head and a merge layer on top. """,
    ALBERT_START_DOCSTRING,
)
class AlbertForSequenceClassification_MERGE(AlbertPreTrainedModel):
    def __init__(self, config):
        super(AlbertForSequenceClassification_MERGE, self).__init__(config)
        self.num_labels = config.num_labels
